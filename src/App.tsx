...
import { parseState, spendYen, toQuery, type AppState } from './lib/urlState'
...
export default function App() {
  ...
  const rows = useMemo(() => breakevens(input.spend, input.assumptions, result.goals), [input, result.goals])
    ...
    <div className="mx-auto max-w-6xl px-4 pt-6 pb-10 sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-stone-600 dark:text-stone-400">
          事業の支払い（送料・仕入れ）を、どのカードで払うと一番得か。年間の支払い額を入れると、年会費を引いた価値で組み合わせを比べます。
        </p>
      ...
      <div className="mt-5 space-y-5">
        ...
        <PerkSection />
        <GoalAnswers answers={answers} active={result.goals} />
